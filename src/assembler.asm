; ------------ CONSTANTES ------------ 
CHAR_EOF K /0000; fim de arquivo
CHAR_LF K /000A; fim de linha ('\n')

; ------------ VARIÁVEIS ------------ 
BOOL_READFILE K /0000; ler ou não o arquivo
BOOL_EOF K /0000; fim do arquivo
BOOL_LF K /0000; quebra de linha

; ------------ MNEUMÔNICOS ------------
GD K /0300; ler byte do arquivo (get data)

; ------------ INÍCIO DO PROGRAMA ------------ 
@ /0000; entry point

; ------------ SUBROTINA PRINT CONSTANTES ------------ 

; ------------ SUBROTINA PRINT VARIÁVEIS ------------ 

; ------------ CODIGO BAGUNCADO ------------ 
@ /0000
INI GD /0300
MM DADO
JZ EOF 
LD DADO
STORE K /9200
LD STORE
AD DOIS
MM STORE
JP INI
EOF SC SAVE_LABELS; subrotina q passa por todas as labels e armazena suas posicoes
SC MNEM2OP
SC ADD_VAR
SC PRINT
FIM HM /0000

PRINT K /0000
LOOP_PRINT K /8400
JZ RS_PRINT
LD POS_M
MM TO_CONVERT
MM MEM_PRINT
LV /0001
MM BOOL
SC CONVERT_TO_PRINT
LD PRINT1
PD /0301
LD PRINT2
PD /0301
GET_COMMANDO K /8400
;JZ RS_PRINT
MM TO_CONVERT
LV /0000
MM BOOL
LD SPACE
PD /0301
SC CONVERT_TO_PRINT
LD PRINT1
PD /0301
LD PRINT2
PD /0301
LD PRINT1
LD EOL_PRINT
PD /0301
LD GET_COMMANDO
AD DOIS
MM GET_COMMANDO
LD LOOP_PRINT
AD DOIS
MM LOOP_PRINT
LD POS_M
AD DOIS
MM POS_M
LV /0000
ZERANDO K /9400
LD ZERANDO
AD DOIS
MM ZERANDO
JP LOOP_PRINT
RS_PRINT RS PRINT


ADD_VAR K /0000
LOOP_VAR LD CONT_OP
JZ RS_ADDVAR
GET_VAR K /8208
MM VAR_ASC
FIND_VAR  K /8340
SB VAR_ASC
JZ FOUND_VAR
LD FIND_VAR
AD QUA
MM FIND_VAR
JP FIND_VAR
FOUND_VAR LD FIND_VAR
AD DOIS
MM GET_VAR_POS
GET_VAR_POS K /8000
MM VAR_POS
OP_LIST K /8400
ML MIL
AD VAR_POS
SAVE_COMANDO K /9400
LD SAVE_COMANDO 
AD DOIS
MM SAVE_COMANDO 
LD OP_LIST
AD DOIS
MM OP_LIST
LD CONT_OP 
SB UM
MM CONT_OP
NEXT_VAR K /8208; vai ate proxima linha pra pegar o valor da proxima var
SB EOL 
JZ FOUND_EOL
LD NEXT_VAR
AD UM
MM NEXT_VAR
JP NEXT_VAR
FOUND_EOL LD NEXT_VAR 
AD AA
MM GET_VAR
MM NEXT_VAR	
LD RESET_FINDVAR
MM FIND_VAR
JP LOOP_VAR
RS_ADDVAR RS ADD_VAR

SAVE_LABELS K /0000 ; Subrotina q percorre todo o codigo e guarda todas as labels e suas posicoes
GET_LABEL K /8200
JZ RS_LABEL
SAVE_L_ASC K /9340
LD POS_L ; posicao de memoria que a label aparece no codigo analisado
SAVE_L_POS K /9342
AD DOIS
MM POS_L
LD SAVE_L_ASC  
AD QUA
MM SAVE_L_ASC
LD SAVE_L_POS
AD QUA
MM SAVE_L_POS
LD GET_LABEL
MM NEXT_LABEL
NEXT_LABEL K /0000 ; vai ate proxima linha pra pegar o valor da proxima label
SB EOL 
JZ ACHOU_EOL
LD NEXT_LABEL
AD UM
MM NEXT_LABEL
JP NEXT_LABEL
ACHOU_EOL LD NEXT_LABEL
AD DOIS
MM GET_LABEL
JP GET_LABEL 
RS_LABEL RS SAVE_LABELS

		MNEM2OP K /0000
		GET_MNEM K /8204 ; 8215
		JZ RS_MNEM2OP
		MM MNEM
LOOP	LD MNEM 
POSM	K /5300
		JZ ACHOU
		LD POSM
		AD DOIS
		MM POSM
		LD POSO
		AD UM
		MM POSO
		JP LOOP
ACHOU   LD POSO
		ML DOIS
		AD GET_OP
		MM PP	
PP		K /0000
		SALVA_OPCODE K /9400; qnd opcode == 0 nao esta avancando a posicao do salva opcode
		LD POSM_O
		MM POSM
		LD ZERO
		MM POSO
		LD SALVA_OPCODE
		AD DOIS
		MM SALVA_OPCODE
		LD GET_MNEM
		MM NEXT_MNEM
		NEXT_MNEM K /0000 
		SB EOL 
		JZ ACHOU_EOL_M
		LD NEXT_MNEM
		AD UM
		MM NEXT_MNEM
		JP NEXT_MNEM
		ACHOU_EOL_M LD NEXT_MNEM
		AD SEIS
		MM GET_MNEM
		LD CONT_OP
		AD UM 
		MM CONT_OP
		JP GET_MNEM
		RS_MNEM2OP RS MNEM2OP

@ /0800
;essa funcao ta zerando a lista de comandos (0400) por algum motivo
CONVERT_TO_PRINT K /0000
LD BOOL
JZ GET_DESLOCADO
K /84FF
JP RETORNO
GET_DESLOCADO K /83FF
;TESTE1 K /9450
;LD TESTE1
;AD DOIS
;MM TESTE1
RETORNO MM VAR_AUX
DV DEZ
MM HEX0
MM HEX
SC HEX2ASC
LD ASC 
MM ASC0

LD HEX0
ML DEZ
MM VAR_AUX0
LD VAR_AUX
SB VAR_AUX0
MM HEX1
MM HEX
SC HEX2ASC
LD ASC 
MM ASC1

LD VAR_AUX
ML CEM
MM VAR_AUX0
LD TO_CONVERT
SB VAR_AUX0
MM VAR_AUX1
DV DEZ
MM HEX2
MM HEX
SC HEX2ASC
LD ASC 
MM ASC2

LD HEX2
ML DEZ
MM VAR_AUX2
LD VAR_AUX1
SB VAR_AUX2
MM HEX3
MM HEX
SC HEX2ASC
LD ASC
MM ASC3

LD ASC0
ML CEM
AD ASC1
MM PRINT1
LD ASC2
ML CEM
AD ASC3
MM PRINT2
LD BOOL
JZ AVANCA_CONT
JP RS_CONVRT_TO_PRINT
AVANCA_CONT LD GET_DESLOCADO
AD DOIS
MM GET_DESLOCADO
RS_CONVRT_TO_PRINT RS CONVERT_TO_PRINT 


HEX2ASC K /0000 ; CONVERSOR DE HEX PARA ASCII
LD HEX
SB NOVE
JZ MENOR_IGUAL9
JN MENOR_IGUAL9; if HEX <= 9
JP MAIOR_9;		 if HEX > 9
MENOR_IGUAL9 LD HEX
AD TRINTA
MM ASC
JP RS_HEX2ASC
MAIOR_9 LD HEX
SB AA
JZ IGUAL_A
LD HEX
SB BB
JZ IGUAL_B
LD HEX 
SB CC
JZ IGUAL_C
LD HEX
SB DD
JZ IGUAL_D
LD HEX 
SB EE
JZ IGUAL_E
LD F_ASC
JP RS_HEX2ASC
IGUAL_A LD A_ASC
JP RS_HEX2ASC
IGUAL_B LD B_ASC
JP RS_HEX2ASC
IGUAL_C LD C_ASC
JP RS_HEX2ASC
IGUAL_D LD D_ASC
JP RS_HEX2ASC
IGUAL_E LD E_ASC
JP RS_HEX2ASC
RS_HEX2ASC MM ASC
RS HEX2ASC 



@ /0500
MEM_PRINT K /0000
BOOL K /0000
DADO K /0000
POS_L K /0000
UM K /0001
DOIS K /0002
QUA K /0004
LABEL K /0000
OITOS K /8000
NOVES K /9000
NOVE K /0009
EOL K /0D0A
MNEM K /0000
POSO K /0000
OPCODE K /0000
GET_OP K /8320
MIL K /1000
POSM_O K /5300
SEIS K /0006
ZERO K /0000
TO_CONVERT K /0000
ASC K /0000
ASC0 K /0000
ASC1 K /0000
ASC2 K /0000
ASC3 K /0000
PRINT1 K /0000
PRINT2 K /0000
POS_M K /0000

HEX K /0000; HEX sera tratado como um vetor composto por = (HEX0, HEX1, HEX2, HEX3)
HEX0 K /0000
HEX1 K /0000
HEX2 K /0000
HEX3 K /0000
TRINTA K /0030
AA K /000A
BB K /000B
CC K /000C
DD K /000D
EE K /000E
FF K /000F
A_ASC K /0041
B_ASC K /0042
C_ASC K /0043
D_ASC K /0044
E_ASC K /0045
F_ASC K /0046
CEM K /0100
DEZ K /0010
CONT_OP K /0000
VAR_ASC K /0000
VAR_POS K /0000
CONT_OP_LOOP K /0000
OITO K /0008
RESET_FINDVAR K /8340
RESET_LOOP_EOL_VAR K /8208
M_DEZ K /FFF6
M_CEM K /FF9C
M_MIL K /FC18
CON_VAR K /0000
SPACE K /2020
VAR_AUX K /0000
VAR_AUX0 K /0000
VAR_AUX1 K /0000
VAR_AUX2 K /0000
EOL_PRINT K /200A

;texto do codigo em ascII guardado a partir da posicao 0200
;lista de tags(em ascII) e suas posicoes, guardados a partir de 0340
;lista de opcodes guardados (ja convertidos) a partir de 0400 


TABELA	@ /0300 ;Tabela OPCODE2MNEM
		 K /4A50 ; 0 JP 20
		 K /4A5A ; 1 JZ 22
		 K /4A4E ; 2 JN 24 
		 K /4C56 ; 3 LV 26
		 K /4144 ; 4 AD 28
		 K /5342 ; 5 SB 2A
		 K /4D4C ; 6 ML 2C
		 K /4456 ; 7 DV 2E
		 K /4C44 ; 8 LD 30
		 K /4D4D ; 9 MM 32
		 K /5343 ; A SC 34
		 K /5253 ; B RS 36
		 K /484D ; C HM 38 
		 K /4744 ; D GD 3A
		 K /5044 ; E PD 3C
		 K /4F53 ; F OS 3E
		 @ /0320
		 ;Tabela MNEM2OPCODE
		 K /0000 ; JP 40
		 K /0001 ; JZ 42
		 K /0002 ; JN 44
		 K /0003 ; LV 46
		 K /0004 ; AD 48
		 K /0005 ; SB 4A
		 K /0006 ; ML 4C
		 K /0007 ; DV 4E
		 K /0008 ; LD 50
		 K /0009 ; MM 52 
		 K /000A ; SC 54
		 K /000B ; RS 56 
		 K /000C ; HM 58 
		 K /000D ; GD 5A
		 K /000E ; PD 5C
		 K /000F ; OS 5E
;@ /0340 guardar as tags a partir de 0340
; ------------ SUBROTINA ------------ 
; @ /0100
; FIM_LINHA JZ EOF 
; MN CHAR_LF
; JZ LF
; EOF LV /0001
; MM BOOL_EOF
; LF LV /0001
; MM BOOL_LF   
; FIM_LI

